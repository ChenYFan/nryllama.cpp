...
void llama_model_glm_dsa::load_arch_tensors(llama_model_loader &) {
    ...
    for (int i = 0; i < n_layer_all; ++i) {
        ...
        // DSA indexer
        // NOTE: GLM_DSA reuses deepseek2's dense-MLA graph (`using graph =
        // llama_model_deepseek2::graph`), which does NOT use the indexer. GLM-5.2
        // only ships indexer weights on "full" (indexer_types) layers; "shared"
        // layers reuse them and have none. Mark NOT_REQUIRED so shared layers
        // load (full-layer weights kept for a future real DSA graph).
